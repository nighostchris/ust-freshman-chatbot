package com.cse3111project.bot.spring.category.academic;

//import com.cse3111project.bot.spring.model.engine.DatabaseEngine;
import com.cse3111project.bot.spring.model.engine.marker.SQLAccessible;
import com.cse3111project.bot.spring.model.engine.SQLDatabaseEngine;
import com.cse3111project.bot.spring.model.engine.marker.StaticAccessible;
import com.cse3111project.bot.spring.model.engine.StaticDatabaseEngine;

import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

import java.net.URISyntaxException;

import java.util.Scanner;
import java.util.ArrayList;
import com.cse3111project.bot.spring.utility.Utilities;

import com.cse3111project.bot.spring.exception.NotSQLAccessibleError;
import com.cse3111project.bot.spring.exception.NotStaticAccessibleError;
import com.cse3111project.bot.spring.exception.StaticDatabaseFileNotFoundException;

import lombok.extern.slf4j.Slf4j;

//Course Category
//SQL database, static database are available
@Slf4j

public class Course extends Academic implements SQLAccessible, StaticAccessible{
	public static final String COURSE_CODE_KEYWORD[];

    // format:
    // id course_code course_title course_credit prerequisite exclusion corequisite
    private static final String SQL_TABLE = "hkust_courses";
    private static final String STATIC_TABLE = "/static/academic/courseDatabase.txt";

    // temporarily just consider queried course code
    private ArrayList<String> userQuery;
    private ArrayList<CourseInfo> results;

    Course(final ArrayList<String> userQuery) {
        this.userQuery = userQuery;
    }

    // initialize QUERY_KEYWORD from SQL database
    static {
        COURSE_CODE_KEYWORD = new String[] { 
        									"ACCT 1010",
							        		"ACCT 2010",
							        		"ACCT 2020",
							        		"ACCT 2200",
							        		"ACCT 3010",
							        		"ACCT 3020",
							        		"ACCT 3030",
							        		"ACCT 3210",
							        		"ACCT 3610",
							        		"ACCT 3650",
							        		"ACCT 3880",
							        		"ACCT 4010",
							        		"ACCT 4020",
							        		"ACCT 4410",
							        		"ACCT 4510",
							        		"ACCT 4610",
							        		"ACCT 4980",
							        		"BIBU 1010",
							        		"BIEN 2010",
							        		"BIEN 3010",
							        		"BIEN 3020",
							        		"BIEN 4910",
							        		"BIPH 2010",
							        		"BIPH 3010",
							        		"BIPH 4010",
							        		"BMED 1100",
							        		"BMED 4950",
							        		"CENG 1000",
							        		"CENG 1010",
							        		"CENG 1500",
							        		"CENG 1600",
							        		"CENG 1700",
							        		"CENG 1800",
							        		"CENG 1980",
							        		"CENG 1990",
							        		"CENG 2110",
							        		"CENG 2210",
							        		"CENG 2220",
							        		"CENG 3120",
							        		"CENG 3210",
							        		"CENG 3220",
							        		"CENG 3230",
							        		"CENG 3910",
							        		"CENG 3920",
							        		"CENG 3926",
							        		"CENG 3927",
							        		"CENG 4000",
							        		"CENG 4020",
							        		"CENG 4120",
							        		"CENG 4130",
							        		"CENG 4140",
							        		"CENG 4540",
							        		"CENG 4620",
							        		"CENG 4630",
							        		"CENG 4640",
							        		"CENG 4660",
							        		"CENG 4670",
							        		"CENG 4710",
							        		"CENG 4720",
							        		"CENG 4911",
							        		"CENG 4912",
							        		"CENG 4913",
							        		"CENG 4970",
							        		"CENG 4980",
							        		"CHEM 1002",
							        		"CHEM 1004",
							        		"CHEM 1008",
							        		"CHEM 1010",
							        		"CHEM 1020",
							        		"CHEM 1030",
							        		"CHEM 1050",
							        		"CHEM 1055",
							        		"CHEM 2110",
							        		"CHEM 2111",
							        		"CHEM 2150",
							        		"CHEM 2151",
							        		"CHEM 2155",
							        		"CHEM 2210",
							        		"CHEM 2250",
							        		"CHEM 2310",
							        		"CHEM 2311",
							        		"CHEM 2350",
							        		"CHEM 2355",
							        		"CHEM 2410",
							        		"CHEM 2418",
							        		"CHEM 2450",
							        		"CHEM 2578",
							        		"CHEM 3010",
							        		"CHEM 3120",
							        		"CHEM 3220",
							        		"CHEM 3320",
							        		"CHEM 3420",
							        		"CHEM 3550",
							        		"CHEM 3555",
							        		"CHEM 3568",
							        		"CHEM 3588",
							        		"CHEM 3610",
							        		"CHEM 3918",
							        		"CHEM 3928",
							        		"CHEM 4110",
							        		"CHEM 4120",
							        		"CHEM 4130",
							        		"CHEM 4140",
							        		"CHEM 4150",
							        		"CHEM 4155",
							        		"CHEM 4210",
							        		"CHEM 4220",
							        		"CHEM 4230",
							        		"CHEM 4240",
							        		"CHEM 4250",
							        		"CHEM 4255",
							        		"CHEM 4310",
							        		"CHEM 4320",
							        		"CHEM 4330",
							        		"CHEM 4340",
							        		"CHEM 4350",
							        		"CHEM 4355",
							        		"CHEM 4430",
							        		"CHEM 4550",
							        		"CHEM 4555",
							        		"CHEM 4620",
							        		"CHEM 4640",
							        		"CHEM 4680",
							        		"CHEM 4688",
							        		"CHEM 4689",
							        		"CHEM 4691",
							        		"CHEM 4692",
							        		"CIVL 1010",
							        		"CIVL 1020",
							        		"CIVL 1030",
							        		"CIVL 1100",
							        		"CIVL 1120",
							        		"CIVL 1130",
							        		"CIVL 1140",
							        		"CIVL 1160",
							        		"CIVL 1170",
							        		"CIVL 2010",
							        		"CIVL 2020",
							        		"CIVL 2110",
							        		"CIVL 2120",
							        		"CIVL 2150",
							        		"CIVL 2160",
							        		"CIVL 2170",
							        		"CIVL 2410",
							        		"CIVL 2510",
							        		"CIVL 2810",
							        		"CIVL 3010",
							        		"CIVL 3020",
							        		"CIVL 3210",
							        		"CIVL 3220",
							        		"CIVL 3310",
							        		"CIVL 3320",
							        		"CIVL 3420",
							        		"CIVL 3510",
							        		"CIVL 3520",
							        		"CIVL 3530",
							        		"CIVL 3610",
							        		"CIVL 3620",
							        		"CIVL 3720",
							        		"CIVL 3730",
							        		"CIVL 3740",
							        		"CIVL 4100",
							        		"CIVL 4230",
							        		"CIVL 4250",
							        		"CIVL 4260",
							        		"CIVL 4270",
							        		"CIVL 4320",
							        		"CIVL 4330",
							        		"CIVL 4340",
							        		"CIVL 4350",
							        		"CIVL 4370",
							        		"CIVL 4380",
							        		"CIVL 4430",
							        		"CIVL 4440",
							        		"CIVL 4450",
							        		"CIVL 4460",
							        		"CIVL 4470",
							        		"CIVL 4520",
							        		"CIVL 4540",
							        		"CIVL 4620",
							        		"CIVL 4700",
							        		"CIVL 4710",
							        		"CIVL 4750",
							        		"CIVL 4760",
							        		"CIVL 4810",
							        		"CIVL 4900",
							        		"CIVL 4910",
							        		"CIVL 4920",
							        		"CIVL 4950",
							        		"CIVL 4990",
							        		"COMP 1001",
							        		"COMP 1021",
							        		"COMP 1022P",
							        		"COMP 1022Q",
							        		"COMP 1029A",
							        		"COMP 1029C",
							        		"COMP 1029J",
							        		"COMP 1029P",
							        		"COMP 1029V",
							        		"COMP 1900",
							        		"COMP 1941",
							        		"COMP 1942",
							        		"COMP 1991",
							        		"COMP 1999",
							        		"COMP 2011",
							        		"COMP 2012",
							        		"COMP 2012H",
							        		"COMP 2021",
							        		"COMP 2521",
							        		"COMP 2611",
							        		"COMP 2711",
							        		"COMP 2711H",
							        		"COMP 2900",
							        		"COMP 3021",
							        		"COMP 3031",
							        		"COMP 3071",
							        		"COMP 3111",
							        		"COMP 3111H",
							        		"COMP 3211",
							        		"COMP 3311",
							        		"COMP 3511",
							        		"COMP 3632",
							        		"COMP 3711",
							        		"COMP 3711H",
							        		"COMP 3721",
							        		"COMP 3900",
							        		"COMP 4021",
							        		"COMP 4111",
							        		"COMP 4211",
							        		"COMP 4221",
							        		"COMP 4311",
							        		"COMP 4321",
							        		"COMP 4331",
							        		"COMP 4332",
							        		"COMP 4411",
							        		"COMP 4421",
							        		"COMP 4431",
							        		"COMP 4441",
							        		"COMP 4451",
							        		"COMP 4461",
							        		"COMP 4511",
							        		"COMP 4521",
							        		"COMP 4611",
							        		"COMP 4621",
							        		"COMP 4622",
							        		"COMP 4631",
							        		"COMP 4632",
							        		"COMP 4641",
							        		"COMP 4651",
							        		"COMP 4900",
							        		"COMP 4901",
							        		"COMP 4911",
							        		"COMP 4971",
							        		"COMP 4981",
							        		"COMP 4981H",
							        		"COMP 4982",
							        		"COMP 4982H",
							        		"COMP 4988",
							        		"COMP 4989",
							        		"COMP 4991",
							        		"COMP 4992",
							        		"CPEG 1971",
							        		"CPEG 2930",
							        		"CPEG 3930",
							        		"ECON 1503",
							        		"ECON 2000",
							        		"ECON 2103",
							        		"ECON 2113",
							        		"ECON 2123",
							        		"ECON 2152",
							        		"ECON 2174",
							        		"ECON 2310",
							        		"ECON 3014",
							        		"ECON 3024",
							        		"ECON 3113",
							        		"ECON 3123",
							        		"ECON 3133",
							        		"ECON 3143",
							        		"ECON 3334",
							        		"ECON 4114",
							        		"ECON 4124",
							        		"ECON 4204",
							        		"ECON 4234",
							        		"ECON 4254",
							        		"ECON 4274",
							        		"ECON 4284",
							        		"ECON 4304",
							        		"ECON 4334",
							        		"ECON 4364",
							        		"ECON 4374",
							        		"ECON 4384",
							        		"ECON 4394",
							        		"ECON 4434",
							        		"ECON 4474",
							        		"ECON 4584",
							        		"ECON 4670",
							        		"ECON 4959",
							        		"ECON 4999",
							        		"ELEC 1010",
							        		"ELEC 1020",
							        		"ELEC 1100",
							        		"ELEC 1200",
							        		"ELEC 1970",
							        		"ELEC 1980",
							        		"ELEC 1990",
							        		"ELEC 1991",
							        		"ELEC 2100",
							        		"ELEC 2100H",
							        		"ELEC 2200",
							        		"ELEC 2300",
							        		"ELEC 2400",
							        		"ELEC 2420",
							        		"ELEC 2500",
							        		"ELEC 2600",
							        		"ELEC 2600H",
							        		"ELEC 2910",
							        		"ELEC 3100",
							        		"ELEC 3200",
							        		"ELEC 3300",
							        		"ELEC 3400",
							        		"ELEC 3500",
							        		"ELEC 3600",
							        		"ELEC 3710",
							        		"ELEC 3910",
							        		"ELEC 4010",
							        		"ELEC 4110",
							        		"ELEC 4120",
							        		"ELEC 4130",
							        		"ELEC 4140",
							        		"ELEC 4150",
							        		"ELEC 4160",
							        		"ELEC 4170",
							        		"ELEC 4220",
							        		"ELEC 4310",
							        		"ELEC 4320",
							        		"ELEC 4330",
							        		"ELEC 4410",
							        		"ELEC 4420",
							        		"ELEC 4430",
							        		"ELEC 4510",
							        		"ELEC 4520",
							        		"ELEC 4530",
							        		"ELEC 4610",
							        		"ELEC 4620",
							        		"ELEC 4630",
							        		"ELEC 4810",
							        		"ELEC 4820",
							        		"ELEC 4900",
							        		"ELEC 4901",
							        		"ELEC 4903",
							        		"ELEC 4904",
							        		"ELEC 4907",
							        		"ELEC 4908",
							        		"ELEC 4909",
							        		"ELEC 4918",
							        		"ELEC 4919",
							        		"ELEC 4930",
							        		"ELEC 4940",
							        		"ELEC 4950",
							        		"ENGG 1010",
							        		"ENGG 1100",
							        		"ENGG 1110",
							        		"ENGG 1130",
							        		"ENGG 1150",
							        		"ENGG 1200",
							        		"ENGG 1900",
							        		"ENGG 2010",
							        		"ENGG 2201",
							        		"ENGG 2900",
							        		"ENGG 2990",
							        		"ENGG 2991-2993",
							        		"ENGG 3000",
							        		"ENGG 3960",
							        		"ENGG 4901",
							        		"ENGG 4902",
							        		"ENGG 4903",
							        		"ENGG 4904",
							        		"ENGG 4950",
							        		"ENTR 2010",
							        		"ENTR 3010",
							        		"ENTR 3020",
							        		"ENTR 3030",
							        		"ENTR 3100",
							        		"ENTR 4000",
							        		"ENTR 4901-4904",
							        		"ENVR 1001",
							        		"ENVR 1030",
							        		"ENVR 1040",
							        		"ENVR 1050",
							        		"ENVR 1070",
							        		"ENVR 1080",
							        		"ENVR 1170",
							        		"ENVR 1811",
							        		"ENVR 2001",
							        		"ENVR 2002",
							        		"ENVR 2010",
							        		"ENVR 2020",
							        		"ENVR 2030",
							        		"ENVR 2310",
							        		"ENVR 2811",
							        		"ENVR 2900",
							        		"ENVR 3001",
							        		"ENVR 3002",
							        		"ENVR 3003",
							        		"ENVR 3010",
							        		"ENVR 3110",
							        		"ENVR 3210",
							        		"ENVR 3220",
							        		"ENVR 3310",
							        		"ENVR 3410",
							        		"ENVR 3420",
							        		"ENVR 4000",
							        		"ENVR 4001",
							        		"ENVR 4010",
							        		"ENVR 4220",
							        		"ENVR 4310",
							        		"ENVR 4320",
							        		"ENVR 4330",
							        		"ENVR 4800",
							        		"ENVR 4980",
							        		"ENVR 4990",
							        		"ENVS 2001",
							        		"ENVS 2002",
							        		"ENVS 2003",
							        		"ENVS 2004",
							        		"ENVS 3001",
							        		"ENVS 3004",
							        		"ENVS 3005",
							        		"ENVS 4001",
							        		"ENVS 4012",
							        		"ENVS 4301",
							        		"ENVS 4905",
							        		"ENVS 4964",
							        		"ENVS 4974",
							        		"ENVS 4984",
							        		"FINA 1203",
							        		"FINA 1303",
							        		"FINA 2102",
							        		"FINA 2203",
							        		"FINA 2303",
							        		"FINA 3001",
							        		"FINA 3011",
							        		"FINA 3103",
							        		"FINA 3103H",
							        		"FINA 3203",
							        		"FINA 3203H",
							        		"FINA 3303",
							        		"FINA 3303H",
							        		"FINA 3403",
							        		"FINA 3403H",
							        		"FINA 3810",
							        		"FINA 3820",
							        		"FINA 3830",
							        		"FINA 4003",
							        		"FINA 4103",
							        		"FINA 4203",
							        		"FINA 4303",
							        		"FINA 4403",
							        		"FINA 4503",
							        		"FINA 4603",
							        		"FINA 4713",
							        		"FINA 4803",
							        		"FINA 4919",
							        		"FINA 4929",
							        		"FYTG 4001",
							        		"GBUS 1000",
							        		"GBUS 2010",
							        		"GBUS 2020",
							        		"GBUS 3010",
							        		"GBUS 3020",
							        		"GBUS 3030",
							        		"GBUS 3040",
							        		"GBUS 3050",
							        		"GBUS 4000",
							        		"GBUS 4010",
							        		"GBUS 4910",
							        		"GNED 2000",
							        		"GNED 2510",
							        		"GNED 2710",
							        		"HART 1001",
							        		"HART 1011",
							        		"HART 1012",
							        		"HART 1013",
							        		"HART 1014",
							        		"HART 1015",
							        		"HART 1016",
							        		"HART 1017",
							        		"HART 1018",
							        		"HART 1019",
							        		"HART 1020",
							        		"HART 1021",
							        		"HART 1022",
							        		"HART 1023",
							        		"HART 1024",
							        		"HART 1025",
							        		"HART 1026",
							        		"HART 1027",
							        		"HART 1028",
							        		"HART 1029",
							        		"HART 1030",
							        		"HART 1031",
							        		"HART 1032",
							        		"HART 1033",
							        		"HART 1034",
							        		"HART 1035",
							        		"HART 1036",
							        		"HART 1037",
							        		"HART 1038",
							        		"HART 1039",
							        		"HART 1040",
							        		"HART 1041",
							        		"HART 1042",
							        		"HART 1043",
							        		"HART 1044",
							        		"HART 1045",
							        		"HLTH 1010",
							        		"HUMA 1000",
							        		"HUMA 1001",
							        		"HUMA 1009",
							        		"HUMA 1010",
							        		"HUMA 1020",
							        		"HUMA 1030",
							        		"HUMA 1060",
							        		"HUMA 1100",
							        		"HUMA 1102",
							        		"HUMA 1210",
							        		"HUMA 1231",
							        		"HUMA 1250",
							        		"HUMA 1300",
							        		"HUMA 1430",
							        		"HUMA 1440",
							        		"HUMA 1520",
							        		"HUMA 1620",
							        		"HUMA 1630",
							        		"HUMA 1631",
							        		"HUMA 1639",
							        		"HUMA 1650",
							        		"HUMA 1660",
							        		"HUMA 1710",
							        		"HUMA 1720",
							        		"HUMA 1810",
							        		"HUMA 1811",
							        		"HUMA 1910",
							        		"HUMA 1920",
							        		"HUMA 2000-2001",
							        		"HUMA 2010",
							        		"HUMA 2031",
							        		"HUMA 2050",
							        		"HUMA 2103",
							        		"HUMA 2104",
							        		"HUMA 2105",
							        		"HUMA 2106",
							        		"HUMA 2200",
							        		"HUMA 2210",
							        		"HUMA 2220",
							        		"HUMA 2240",
							        		"HUMA 2250",
							        		"HUMA 2260",
							        		"HUMA 2280",
							        		"HUMA 2300",
							        		"HUMA 2310",
							        		"HUMA 2320",
							        		"HUMA 2400",
							        		"HUMA 2420",
							        		"HUMA 2430",
							        		"HUMA 2440",
							        		"HUMA 2470",
							        		"HUMA 2581",
							        		"HUMA 2589",
							        		"HUMA 2590",
							        		"HUMA 2621",
							        		"HUMA 2623",
							        		"HUMA 2630",
							        		"HUMA 2632",
							        		"HUMA 2633",
							        		"HUMA 2635",
							        		"HUMA 2638",
							        		"HUMA 2640",
							        		"HUMA 2660",
							        		"HUMA 2661",
							        		"HUMA 2670",
							        		"HUMA 2680",
							        		"HUMA 2820",
							        		"HUMA 2830",
							        		"HUMA 2840",
							        		"HUMA 2911",
							        		"HUMA 2921",
							        		"HUMA 3000",
							        		"HUMA 3030",
							        		"HUMA 3040",
							        		"HUMA 3101",
							        		"HUMA 3102",
							        		"HUMA 3103",
							        		"HUMA 3104",
							        		"HUMA 3200",
							        		"HUMA 3201",
							        		"HUMA 3202",
							        		"HUMA 3203",
							        		"HUMA 3210",
							        		"HUMA 3220",
							        		"HUMA 3240",
							        		"HUMA 3250",
							        		"HUMA 3410",
							        		"HUMA 3420",
							        		"HUMA 3430",
							        		"HUMA 3450",
							        		"HUMA 3610",
							        		"HUMA 3630",
							        		"HUMA 3660",
							        		"HUMA 3680",
							        		"HUMA 3800",
							        		"HUMA 3810",
							        		"HUMA 3821",
							        		"HUMA 3900",
							        		"HUMA 4000",
							        		"HUMA 4020",
							        		"HUMA 4220",
							        		"HUMA 4250",
							        		"HUMA 4520",
							        		"HUMA 4610",
							        		"HUMA 4650",
							        		"HUMA 4700",
							        		"HUMA 4840",
							        		"IDPO 4950",
							        		"IELM 1010",
							        		"IELM 1020",
							        		"IELM 1990",
							        		"IELM 1991",
							        		"IELM 2010",
							        		"IELM 2100",
							        		"IELM 2145",
							        		"IELM 2150",
							        		"IELM 2200",
							        		"IELM 2410",
							        		"IELM 2510",
							        		"IELM 3010",
							        		"IELM 3130",
							        		"IELM 3150",
							        		"IELM 3230",
							        		"IELM 3250",
							        		"IELM 3270",
							        		"IELM 3300",
							        		"IELM 3330",
							        		"IELM 3410",
							        		"IELM 3450",
							        		"IELM 3901",
							        		"IELM 4100",
							        		"IELM 4110",
							        		"IELM 4130",
							        		"IELM 4170",
							        		"IELM 4180",
							        		"IELM 4200",
							        		"IELM 4320",
							        		"IELM 4331",
							        		"IELM 4410",
							        		"IELM 4650",
							        		"IELM 4900",
							        		"IELM 4901",
							        		"IELM 4930",
							        		"IELM 4950",
							        		"IELM 4990",
							        		"IIMP 2000",
							        		"IIMP 4980",
							        		"IIMP 4990",
							        		"IROP 1000",
							        		"ISOM 1090",
							        		"ISOM 1380",
							        		"ISOM 1500",
							        		"ISOM 1700",
							        		"ISOM 2010",
							        		"ISOM 2030",
							        		"ISOM 2310",
							        		"ISOM 2400",
							        		"ISOM 2500",
							        		"ISOM 2700",
							        		"ISOM 3000",
							        		"ISOM 3010",
							        		"ISOM 3100",
							        		"ISOM 3180",
							        		"ISOM 3210",
							        		"ISOM 3230",
							        		"ISOM 3260",
							        		"ISOM 3310",
							        		"ISOM 3320",
							        		"ISOM 3360",
							        		"ISOM 3370",
							        		"ISOM 3380",
							        		"ISOM 3530",
							        		"ISOM 3540",
							        		"ISOM 3710",
							        		"ISOM 3730",
							        		"ISOM 3740",
							        		"ISOM 3760",
							        		"ISOM 3900",
							        		"ISOM 4000",
							        		"ISOM 4020",
							        		"ISOM 4100",
							        		"ISOM 4300",
							        		"ISOM 4400",
							        		"ISOM 4490",
							        		"ISOM 4520",
							        		"ISOM 4530",
							        		"ISOM 4540",
							        		"ISOM 4550",
							        		"ISOM 4580",
							        		"ISOM 4590",
							        		"ISOM 4690",
							        		"ISOM 4720",
							        		"ISOM 4740",
							        		"ISOM 4750",
							        		"ISOM 4770",
							        		"ISOM 4780",
							        		"ISOM 4790",
							        		"ISOM 4810",
							        		"ISOM 4820",
							        		"ISOM 4840",
							        		"ISOM 4880",
							        		"ISOM 4890",
							        		"LABU 2020",
							        		"LABU 2040",
							        		"LABU 2050",
							        		"LABU 2051",
							        		"LABU 2052",
							        		"LABU 2060",
							        		"LANG 1002",
							        		"LANG 1003",
							        		"LANG 1113",
							        		"LANG 1114",
							        		"LANG 1115",
							        		"LANG 1116",
							        		"LANG 1117",
							        		"LANG 1118",
							        		"LANG 1120",
							        		"LANG 1121",
							        		"LANG 1122",
							        		"LANG 1123",
							        		"LANG 1124",
							        		"LANG 1125",
							        		"LANG 1126",
							        		"LIFS 3070",
							        		"LANG 1127",
							        		"LANG 1130",
							        		"LANG 1133",
							        		"LANG 1210",
							        		"LANG 1220",
							        		"LANG 1310",
							        		"LANG 1320",
							        		"LANG 1330",
							        		"LANG 1410",
							        		"LANG 2010",
							        		"LANG 2030",
							        		"LANG 2070",
							        		"LANG 2082",
							        		"LANG 2083",
							        		"LANG 2170",
							        		"LANG 3010",
							        		"LANG 3011",
							        		"LANG 3012",
							        		"LANG 3013",
							        		"LANG 3014",
							        		"LANG 3015",
							        		"LANG 3016",
							        		"LANG 3070",
							        		"LANG 3081",
							        		"LANG 4012",
							        		"LANG 4013",
							        		"LANG 4014",
							        		"LANG 4015",
							        		"LANG 4016",
							        		"LANG 4017",
							        		"LANG 4030",
							        		"LANG 4031",
							        		"LANG 4032",
							        		"LANG 4033",
							        		"LANG 4034",
							        		"LANG 4070",
							        		"LIFS 1010",
							        		"LIFS 1020",
							        		"LIFS 1030",
							        		"LIFS 1901",
							        		"LIFS 1902",
							        		"LIFS 1903",
							        		"LIFS 1904",
							        		"LIFS 1930",
							        		"LIFS 2010",
							        		"LIFS 2011",
							        		"LIFS 2040",
							        		"LIFS 2060",
							        		"LIFS 2070",
							        		"LIFS 2080",
							        		"LIFS 2210",
							        		"LIFS 2220",
							        		"LIFS 2240",
							        		"LIFS 2280",
							        		"LIFS 2720",
							        		"LIFS 2820",
							        		"LIFS 3002",
							        		"LIFS 3010",
							        		"LIFS 3020",
							        		"LIFS 3040",
							        		"LIFS 3060",
							        		"LIFS 3110",
							        		"LIFS 3120",
							        		"LIFS 3130",
							        		"LIFS 3140",
							        		"LIFS 3150",
							        		"LIFS 3160",
							        		"LIFS 3210",
							        		"LIFS 3220",
							        		"LIFS 3240",
							        		"LIFS 3260",
							        		"LIFS 3330",
							        		"LIFS 3510",
							        		"LIFS 3520",
							        		"LIFS 3530",
							        		"LIFS 4060",
							        		"LIFS 4090",
							        		"LIFS 4140",
							        		"LIFS 4150",
							        		"LIFS 4170",
							        		"LIFS 4190",
							        		"LIFS 4200",
							        		"LIFS 4360",
							        		"LIFS 4370",
							        		"LIFS 4380",
							        		"LIFS 4540",
							        		"LIFS 4550",
							        		"LIFS 4580",
							        		"LIFS 4620",
							        		"LIFS 4630",
							        		"LIFS 4760",
							        		"LIFS 4800",
							        		"LIFS 4820",
							        		"LIFS 4910",
							        		"LIFS 4950",
							        		"LIFS 4960",
							        		"LIFS 4961",
							        		"LIFS 4963",
							        		"LIFS 4970",
							        		"LIFS 4971",
							        		"LIFS 4972",
							        		"LIFS 4973",
							        		"LIFS 4980",
							        		"LIFS 4981",
							        		"LIFS 4982",
							        		"LIFS 4983",
							        		"MARK 1220",
							        		"MARK 1230",
							        		"MARK 1330",
							        		"MARK 2120",
							        		"MARK 3220",
							        		"MARK 3410",
							        		"MARK 3420",
							        		"MARK 3430",
							        		"MARK 3450",
							        		"MARK 3460",
							        		"MARK 3470",
							        		"MARK 3480",
							        		"MARK 3510",
							        		"MARK 3520",
							        		"MARK 4210",
							        		"MARK 4290",
							        		"MARK 4450",
							        		"MARK 4980",
							        		"MATH 1003",
							        		"MATH 1012",
							        		"MATH 1013",
							        		"MATH 1014",
							        		"MATH 4141",
							        		"MATH 1020",
							        		"MATH 1023",
							        		"MATH 1024",
							        		"MATH 1701",
							        		"MATH 2011",
							        		"MATH 2023",
							        		"MATH 2033",
							        		"MATH 2043",
							        		"MATH 2111",
							        		"MATH 2121",
							        		"MATH 2131",
							        		"MATH 2343",
							        		"MATH 2350",
							        		"MATH 2351",
							        		"MATH 2352",
							        		"MATH 2411",
							        		"MATH 2421",
							        		"MATH 2431",
							        		"MATH 2511",
							        		"MATH 2721",
							        		"MATH 2731",
							        		"MATH 2741",
							        		"MATH 3033",
							        		"MATH 3043",
							        		"MATH 3121",
							        		"MATH 3131",
							        		"MATH 3311",
							        		"MATH 3312",
							        		"MATH 3322",
							        		"MATH 3332",
							        		"MATH 3343",
							        		"MATH 3423",
							        		"MATH 3424",
							        		"MATH 3425",
							        		"MATH 3426",
							        		"MATH 4023",
							        		"MATH 4033",
							        		"MATH 4051",
							        		"MATH 4052",
							        		"MATH 4061",
							        		"MATH 4063",
							        		"MATH 4151",
							        		"MATH 4221",
							        		"MATH 4223",
							        		"MATH 4225",
							        		"MATH 4321",
							        		"MATH 4326",
							        		"MATH 4333",
							        		"MATH 4335",
							        		"MATH 4336",
							        		"MATH 4351",
							        		"MATH 4422",
							        		"MATH 4423",
							        		"MATH 4424",
							        		"MATH 4425",
							        		"MATH 4426",
							        		"MATH 4427",
							        		"MATH 4432",
							        		"MATH 4511",
							        		"MATH 4512",
							        		"MATH 4513",
							        		"MATH 4514",
							        		"MATH 4821",
							        		"MATH 4822",
							        		"MATH 4823",
							        		"MATH 4824",
							        		"MATH 4825",
							        		"MATH 4921",
							        		"MATH 4981-4982",
							        		"MATH 4990",
							        		"MATH 4991",
							        		"MATH 4992",
							        		"MATH 4993",
							        		"MATH 4994",
							        		"MATH 4999",
							        		"MECH 1020",
							        		"MECH 1030",
							        		"MECH 1901",
							        		"MECH 1902",
							        		"MECH 1905",
							        		"MECH 1906",
							        		"MECH 1907",
							        		"MECH 1990",
							        		"MECH 2020",
							        		"MECH 2040",
							        		"MECH 2210",
							        		"MECH 2310",
							        		"MECH 2410",
							        		"MECH 2520",
							        		"MECH 2907",
							        		"MECH 3020",
							        		"MECH 3030",
							        		"MECH 3300",
							        		"MECH 3310",
							        		"MECH 3420",
							        		"MECH 3510",
							        		"MECH 3520",
							        		"MECH 3610",
							        		"MECH 3620",
							        		"MECH 3630",
							        		"MECH 3640",
							        		"MECH 3650",
							        		"MECH 3660",
							        		"MECH 3670",
							        		"MECH 3680",
							        		"MECH 3690",
							        		"MECH 3710",
							        		"MECH 3830",
							        		"MECH 4000",
							        		"MECH 4010",
							        		"MECH 4100",
							        		"MECH 4110",
							        		"MECH 4340",
							        		"MECH 4350",
							        		"MECH 4360",
							        		"MECH 4430",
							        		"MECH 4450",
							        		"MECH 4710",
							        		"MECH 4720",
							        		"MECH 4740",
							        		"MECH 4750",
							        		"MECH 4820",
							        		"MECH 4850",
							        		"MECH 4860",
							        		"MECH 4880",
							        		"MECH 4900",
							        		"MECH 4910",
							        		"MECH 4950",
							        		"MECH 4980",
							        		"MECH 4990",
							        		"MECH 4995",
							        		"MGMT 1110",
							        		"MGMT 1120",
							        		"MGMT 1130",
							        		"MGMT 2010",
							        		"MGMT 2110",
							        		"MGMT 2120",
							        		"MGMT 2130",
							        		"MGMT 3110",
							        		"MGMT 3120",
							        		"MGMT 3130",
							        		"MGMT 3140",
							        		"MGMT 3160",
							        		"MGMT 3170",
							        		"MGMT 4000",
							        		"MGMT 4010",
							        		"MGMT 4210",
							        		"MGMT 4220",
							        		"MGMT 4230",
							        		"MGMT 4240",
							        		"MGMT 4250",
							        		"MGMT 4260",
							        		"PHYS 1001",
							        		"PHYS 1002",
							        		"PHYS 1003",
							        		"PHYS 1005",
							        		"PHYS 1006",
							        		"PHYS 1111",
							        		"PHYS 1112",
							        		"PHYS 1113",
							        		"PHYS 1114",
							        		"PHYS 1115",
							        		"PHYS 1312",
							        		"SCIE 1500",
							        		"SOSC 1460",
							        		"PHYS 1314",
							        		"PHYS 2021",
							        		"PHYS 2022",
							        		"PHYS 2023",
							        		"PHYS 2080",
							        		"PHYS 2090",
							        		"PHYS 2124",
							        		"PHYS 3031",
							        		"PHYS 3032",
							        		"PHYS 3033",
							        		"PHYS 3034",
							        		"PHYS 3036",
							        		"PHYS 3037",
							        		"PHYS 3038",
							        		"PHYS 3040",
							        		"PHYS 3041",
							        		"PHYS 3053",
							        		"PHYS 3071",
							        		"PHYS 3090",
							        		"PHYS 3142",
							        		"PHYS 3152",
							        		"PHYS 3153",
							        		"PHYS 3411",
							        		"PHYS 4050",
							        		"PHYS 4051",
							        		"PHYS 4052",
							        		"PHYS 4053",
							        		"PHYS 4054",
							        		"PHYS 4055",
							        		"PHYS 4058",
							        		"PHYS 4059",
							        		"PHYS 4060",
							        		"PHYS 4071",
							        		"PHYS 4080",
							        		"PHYS 4090",
							        		"PHYS 4151",
							        		"PHYS 4191",
							        		"PHYS 4291",
							        		"PHYS 4411",
							        		"PHYS 4498",
							        		"RMBI 1010",
							        		"RMBI 1020",
							        		"RMBI 2001",
							        		"RMBI 3110",
							        		"RMBI 4000",
							        		"RMBI 4110",
							        		"RMBI 4210",
							        		"RMBI 4220",
							        		"RMBI 4310",
							        		"RMBI 4980",
							        		"RMBI 4990",
							        		"SBMT 1100",
							        		"SBMT 1111",
							        		"SBMT 2100-2110",
							        		"SBMT 2200",
							        		"SBMT 2900",
							        		"SBMT 3100",
							        		"SBMT 3200-3210",
							        		"SBMT 3300",
							        		"SCIE 1000",
							        		"SCIE 1010",
							        		"SCIE 1030",
							        		"SCIE 1050",
							        		"SCIE 1080",
							        		"SCIE 1090",
							        		"SCIE 1100",
							        		"SCIE 1110",
							        		"SCIE 1120",
							        		"SCIE 1130",
							        		"SCIE 2000",
							        		"SCIE 2500",
							        		"SCIE 3100",
							        		"SCIE 3110",
							        		"SCIE 3500",
							        		"SCIE 3900",
							        		"SCIE 4500",
							        		"SCIE 4850",
							        		"SCIE 4860",
							        		"SHSS 1010",
							        		"SHSS 1020",
							        		"SHSS 1030",
							        		"SHSS 3001",
							        		"SHSS 3010",
							        		"SHSS 4520",
							        		"SHSS 4991",
							        		"SHSS 4992",
							        		"SHSS 4993",
							        		"SISP 1101",
							        		"SISP 1102",
							        		"SISP 1103",
							        		"SISP 1104",
							        		"SISP 1105",
							        		"SISP 1301",
							        		"SISP 1302",
							        		"SISP 1303",
							        		"SISP 1304",
							        		"SISP 1305",
							        		"SISP 1306",
							        		"SISP 1307",
							        		"SISP 1308",
							        		"SISP 1309",
							        		"SISP 1310",
							        		"SISP 1311",
							        		"SISP 1312",
							        		"SISP 1313",
							        		"SISP 1314",
							        		"SISP 1315",
							        		"SISP 1501",
							        		"SISP 1502",
							        		"SISP 1503",
							        		"SISP 1504",
							        		"SISP 1505",
							        		"SISP 1506",
							        		"SISP 1507",
							        		"SISP 1508",
							        		"SISP 1509",
							        		"SISP 1510",
							        		"SISP 1701",
							        		"SISP 1702",
							        		"SISP 1703",
							        		"SISP 1704",
							        		"SISP 1705",
							        		"SISP 1706",
							        		"SISP 1707",
							        		"SISP 1708",
							        		"SISP 1709",
							        		"SISP 1710",
							        		"SISP 1711",
							        		"SISP 1901",
							        		"SOSC 1000-1010",
							        		"SOSC 1100",
							        		"SOSC 1110",
							        		"SOSC 1130",
							        		"SOSC 1150",
							        		"SOSC 1160",
							        		"SOSC 1170",
							        		"SOSC 1190",
							        		"SOSC 1200",
							        		"SOSC 1210",
							        		"SOSC 1270",
							        		"SOSC 1300",
							        		"SOSC 1340",
							        		"SOSC 1350",
							        		"SOSC 1420",
							        		"SOSC 1440",
							        		"SOSC 1661",
							        		"SOSC 1662",
							        		"SOSC 1780",
							        		"SOSC 1810",
							        		"SOSC 1840",
							        		"SOSC 1850",
							        		"SOSC 1860",
							        		"SOSC 1960",
							        		"SOSC 1980",
							        		"SOSC 2000",
							        		"SOSC 2010",
							        		"SOSC 2120",
							        		"SOSC 2130",
							        		"SOSC 2140",
							        		"SOSC 2170",
							        		"SOSC 2210",
							        		"SOSC 2280",
							        		"SOSC 2290",
							        		"SOSC 2300",
							        		"SOSC 2310",
							        		"SOSC 2400",
							        		"SOSC 2630",
							        		"SOSC 2640",
							        		"SOSC 2740",
							        		"SOSC 2780",
							        		"SOSC 2960",
							        		"SOSC 2970",
							        		"SOSC 2980",
							        		"SOSC 3000",
							        		"SOSC 3010",
							        		"SOSC 3110",
							        		"SOSC 3120",
							        		"SOSC 3130",
							        		"SOSC 3150",
							        		"SOSC 3240",
							        		"SOSC 3330",
							        		"SOSC 3410",
							        		"SOSC 3520",
							        		"SOSC 3530",
							        		"SOSC 3540",
							        		"SOSC 3600",
							        		"SOSC 3620",
							        		"SOSC 3630",
							        		"SOSC 3700",
							        		"SOSC 3880",
							        		"SOSC 4000",
							        		"SOSC 4100",
							        		"SOSC 4110",
							        		"SOSC 4260",
							        		"SOSC 4270",
							        		"SOSC 4280",
							        		"SOSC 4290",
							        		"SOSC 4310",
							        		"SOSC 4320",
							        		"SOSC 4600",
							        		"SOSC 4610",
							        		"SUST 1000",
							        		"TEMG 1010",
							        		"TEMG 4000",
							        		"TEMG 4600",
							        		"TEMG 4950",
							        		"UROP 1000",
							        		"UROP 1100",
							        		"UROP 2100",
							        		"UROP 3100",
							        		"UROP 4100",
							        		"WBBA 2010",
							        		"WBBA 2020",
							        		"WBBA 4010"
        };
    }
    
    // check whether userQuery contains course code, i.e. wrong format 
    // if so, append to matchedResults
    // NOTE that course code should be found by partial match method in SearchEngine.parse()
    // @param userQuery: omitted symbols (!@#$%...) + toLowerCase()
    public static void containsCourseCode(String userQuery, ArrayList<String> matchedResults){
        
        ArrayList<String> newResults = new ArrayList<>();
        for (String courseCodeKeyword : COURSE_CODE_KEYWORD) {
            if (courseCodeKeyword.toLowerCase().contains(userQuery)){
            	newResults.add(courseCodeKeyword);
            }
        }
        // finally append new results
        for (String newResult : newResults)
            matchedResults.add(newResult);
    } 
    
 // used to wrap course object
    class CourseInfo {
        private String code;
        private String title;
        private String credit;
        private String prerequisite;
        private String exclusion;
        private String corequisite;

        private CourseInfo(String code, String title, String credit, String prerequisite, String exclusion, String corequisite){
            this.code = code;
            this.title = title;
            this.credit = credit;
            this.prerequisite = prerequisite;
            this.exclusion = exclusion;
            this.corequisite = corequisite;
        }

        @Override
        public String toString(){
        	prerequisite = !prerequisite.isEmpty() ? "Prerequisite: " + prerequisite + '\n' : "";
        	exclusion = !exclusion.isEmpty() ? "Exclusion: " + exclusion + '\n' : "";
        	corequisite = !corequisite.isEmpty() ? "Corequisite: " + corequisite + '\n' : ""; 
			return  "Course code: " + code + '\n' + 
		    		"Course title: " + title + '\n' + 
		    		"Course credit: " + credit + '\n' + 
		    		prerequisite + exclusion + corequisite;
        }
    }

    // search by course code(s)
    // return course detail(s) using SQL database
    @Override
    public synchronized String getDataFromSQL() throws NotSQLAccessibleError, URISyntaxException, SQLException {
        try (SQLDatabaseEngine database = new SQLDatabaseEngine(this, SQL_TABLE)) {
            results = new ArrayList<>();

            Utilities.arrayLog("query course", userQuery);
            for (int i = 0; i < userQuery.size(); i++) {
            	StringBuilder SQLStatement = new StringBuilder("SELECT course_code, course_title, course_credit, prerequisite, exclusion, corequisite FROM ")
            			.append(SQL_TABLE).append(" WHERE course_code = ?");

            	PreparedStatement SQLQuery = database.prepare(SQLStatement.toString());

            	SQLQuery.setString(1, userQuery.get(i));
//            	SQLQuery.setString(2, userQuery.get(i));

            	ResultSet reader = database.executeQuery();
            	while (reader.next()){
            		results.add(new CourseInfo(reader.getString(1), reader.getString(2), 
            									reader.getString(3), reader.getString(4),
            									reader.getString(5), reader.getString(6)));
            	}
            }

            // ONLY happens when the SQL database and static database are NOT synchronized
            if (results.isEmpty())
                return "Specified course not found. Sorry";

            return super.replyResults(results);
        }
    }

    // return course detail(s) using static database
    // only used when fail to connect SQL database
    @Override
    public synchronized String getDataFromStatic() throws NotStaticAccessibleError, StaticDatabaseFileNotFoundException {
        try (StaticDatabaseEngine database = new StaticDatabaseEngine(this, STATIC_TABLE)) {
            results = new ArrayList<>();

            Scanner reader = database.executeQuery();

            while (reader.hasNextLine()){
                // format:
            	// id course_code course_title course_credit prerequisite exclusion corequisite
                String line = reader.nextLine();

                if (line.startsWith("#") || line.length() == 0)
                    continue;

                String courseCode = line.split(",")[2];
                for (String query : userQuery){
                    if (query.equals(courseCode))
                        results.add(new CourseInfo(line.split(",")[2], line.split(",")[3],
                                                  line.split(",")[4], line.split(",")[5],
                                                  line.split(",")[6], line.split(",")[7]));
                }
            }

            // occurs only when the static initialized COURSE_CODE_KEYWORD and static database are not sync
            if (results.isEmpty())
                return "Specified course not found. Sorry";

            return super.replyResults(results);
        }
    }
}
